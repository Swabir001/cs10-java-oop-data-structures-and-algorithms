...
public class ThumbnailsHandout extends DrawingGUI {
	...
	public ThumbnailsHandout(ArrayList<BufferedImage> images) {
		thumbs = new ArrayList<>(); //Bug
		initWindow(thumbWidth*tcols, thumbHeight*trows); // bug
...
//	}
	...
	private static BufferedImage thumbnailify(BufferedImage image) {
		...
		for (int y = 0; y < thumbHeight; y++) {
			for (int x = 0; x < thumbWidth; x++) {
				int srcX = x * w / thumbWidth;
				int srcY = y * h / thumbHeight;
				result.setRGB(x, y, image.getRGB(srcX, srcY));
			}
		}
		return result;
	}
	...
	public void draw(Graphics g) {
		for (int i=0; i<trows; i++) { //Bug
			for (int j=0; j<tcols; j++) { //Bug
...
//				ArrayList<BufferedImage> images = new ArrayList<BufferedImage>();
				for (int i=0; i<trows; i++) {
					for (int j=0; j<tcols; j++) {
						images.add(loadImage("PD1/dart/dart"+(i*tcols+j)+".jpg"));
					}
				}
